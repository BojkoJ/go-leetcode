...
func lengthOfLongestSubstring(s string) int {
	...
	// During the reading we constantly update simple integer variable maxLen our currently found maximum
	...
	// When we find character which is already in the map, simply cut the beginning of the window:
	//     - that means move the beginning of the window to the place right behind first occurrence of this duplicit character

	maxLen, currentLen := 0, 0 // biggest and current length found of substríng of unique characters
	m := make(map[rune]int)    // hash map will be saving: character -> index
	// start of our window
	start := 0 // no need to create the "end" of our window - that will be the index in the loop
	...
	for index, character := range s {
		...
		if ok && val >= start {
			...
			// DUPLICATE - so we need to move the start of our window behind the current character
			// this moves the window and creates opportunity to check new substrings
			start = val + 1 // move the start of the window: one character to the right of current
		}
		// we need to save the character to the map every iteration
		// if it's already in the map, this will just update its index
		m[character] = index
		currentLen = index - start + 1 // length of current window is it's end minus it's start
